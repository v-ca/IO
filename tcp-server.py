...
def handle(client: socket) -> None:
    """
    Handles the client connection. This function is called when a new client connects to the server.
        ...
        try:
            ...
            if message.decode("utf-8").startswith("KICK"):
                ...
                if names[clients.index(client)] == "admin":
                    ...
                    print(
                        f"{name_to_ban} has felt the swift justice of the BANHAMMER!!"
                    )
                else:
                    client.send("Command was refused!".encode("utf-8"))
...
def kick_user(name: str) -> None:
    """
    Kick a user from the server. Used by the admin.

    args:
        name: str: The name of the user to kick
